...
// It probes localhost ports for local inference providers on GKE GPU nodes
// (vLLM, Text Generation Inference, self-hosted Model Garden endpoints)
...
const (
	...
	annotationAccelerator = "sympozium.ai/inference-accelerator"
...
func main() {
	...
	// Fetch node labels for accelerator detection.
	node, err := clientset.CoreV1().Nodes().Get(ctx, nodeName, metav1.GetOptions{})
	if err != nil {
		log.Error(err, "failed to get node object", "node", nodeName)
		// Continue anyway; accelerator annotation will be empty
	}
	...
	runProbeLoop(ctx, clientset, nodeName, cfg.Targets, interval, registry, nodeLabels)
}
...
func runProbeLoop(ctx context.Context, clientset kubernetes.Interface, nodeName string, targets []ProbeTarget, interval time.Duration, registry *targetRegistry, nodeLabels map[string]string) {
	...
	if err := patchNodeAnnotations(ctx, clientset, nodeName, results, nodeLabels); err != nil {
...
// Supports vLLM/OpenAI-compatible format ({"data":[{"id":"..."}]}) and
...
// detectAccelerator reads GKE-specific node labels to determine the accelerator type.
// Returns a string like "tpu-v5e", "gpu-nvidia-l4", "gpu-nvidia-a100", or "cpu".
func detectAccelerator(nodeLabels map[string]string) string {
	if nodeLabels == nil {
		return "cpu"
	}

	// Check for TPU first
	if topo, ok := nodeLabels["cloud.google.com/gke-tpu-topology"]; ok && topo != "" {
		accel := nodeLabels["cloud.google.com/gke-tpu-accelerator"]
		if accel == "" {
			accel = "tpu-v5e" // default for GKE TPU nodes
		}
		return accel
	}

	// Check for GPU
	if gpu, ok := nodeLabels["cloud.google.com/gke-accelerator"]; ok && gpu != "" {
		return "gpu-" + gpu
	}
...
func buildAnnotations(results []ProbeResult, nodeLabels map[string]string) map[string]interface{} {
	...
	if anyHealthy {
		...
		// Add accelerator annotation
		accelerator := detectAccelerator(nodeLabels)
		annotations[annotationAccelerator] = accelerator
	} else {
		annotations[annotationHealthy] = nil
		annotations[annotationLastPr] = nil
		annotations[annotationPrefix+"proxy-port"] = nil
		annotations[annotationAccelerator] = nil
	}

	return annotations
}

func patchNodeAnnotations(ctx context.Context, clientset kubernetes.Interface, nodeName string, results []ProbeResult, nodeLabels map[string]string) error {
	annotations := buildAnnotations(results, nodeLabels)
